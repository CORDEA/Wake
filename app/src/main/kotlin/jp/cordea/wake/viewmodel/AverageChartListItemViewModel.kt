 ...
 */
class AverageChartListItemViewModel : DataListItemViewModel() {

    val averageBarData: BarDataSet
        get() {
            ...
            val entries = results.map {
                ...
                BarEntry(it.id.toFloat(), sakes.map { getVolume(it) }.sum() / (getSessionTime(it) / 3600f))
            }
